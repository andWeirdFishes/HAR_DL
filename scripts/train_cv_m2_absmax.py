    ...
    for i in range(win_count):
        ...
        if len(sequence) == 1:
            ...
            if sequence[0] == "upright_still" and sequence[2] == "upright_still":
                ...
                if trans_type == "transition_up":
                    if start_orig == "sitting" and end_orig == "standing":
                        assigned_label = "transition_up"
                elif trans_type == "transition_down":
                    if start_orig == "standing" and end_orig == "sitting":
...
def build_fold_data(
    ...
    for sub in sorted(pre_path.iterdir()):
        ...
        for csv in sorted(sub.rglob("*.csv")):
            ...
            wins, labs = _make_sliding_windows(df, s_cols, w_size, w_step, l2idx, l_col, s_col)
            if len(wins) > 0: 
                f_w[fid].append(wins)
                f_l[fid].append(labs)
...
def _save_loss_plot(hist: list[dict], path: Path, title: str) -> None:
    e, l, a = [h["epoch"] for h in hist], [h["train_loss"] for h in hist], [h["train_acc"] for h in hist]
    fig, ax1 = plt.subplots(figsize=(10, 6))
    ax1.set_xlabel("Epoch")
    ax1.set_ylabel("Loss", color="tab:red")
    ax1.plot(e, l, color="tab:red")
    ax1.tick_params(axis="y", labelcolor="tab:red")
    ax2 = ax1.twinx()
    ax2.set_ylabel("Accuracy", color="tab:blue")
    ax2.plot(e, a, color="tab:blue")
...
def main() -> None:
    ...
    f_a, l_a, l2i = build_fold_data(
        root / "data" / "preprocessed" / "HAR_DL_FEIT_2025", 
