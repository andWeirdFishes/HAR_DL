...
STEP_SIZE = int(
    config["window_slide_sec"] * config["sampling_frequency"]
)  # 1.0 * 50 = 50
...
def main():
    ...
    results_path = Path(get_project_root()) / "data" / "results" / "6fcv_overlap.csv"
    results_path.parent.mkdir(parents=True, exist_ok=True)
    results_df.to_csv(results_path, index=False)
    preds_path = (
        Path(get_project_root()) / "data" / "results" / "6fcv_overlap_preds.csv"
    )
