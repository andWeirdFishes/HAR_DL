...
class AbsMaxPool1d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, C, T = x.shape
        T_out = (T - self.kernel_size) // self.stride + 1
        x_unf = x.unfold(-1, self.kernel_size, self.stride)
        abs_idx = x_unf.abs().argmax(dim=-1, keepdim=True)
        return x_unf.gather(-1, abs_idx).squeeze(-1)
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        if pool:
            layers.append(AbsMaxPool1d(kernel_size=2, stride=2))
